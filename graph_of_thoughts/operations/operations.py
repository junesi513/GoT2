...
class Generate(Operation):
    ...
    def _execute(
        ...
        """
        Executes the generation of new thoughts.
        If the operation has predecessors, it generates new thoughts for each of them.
        Otherwise, it generates a new thought based on the initial problem parameters.
        ...
        :param parser: The parser for parsing responses.
        :type parser: Parser
        :param kwargs: Additional parameters for execution, used as initial state if no predecessors.
        """
        previous_thoughts: List[Thought] = self.get_previous_thoughts()

        if len(previous_thoughts) > 0:
            # Generate thoughts based on predecessors
            for thought in previous_thoughts:
                ...
                for new_state in parser.parse_generate_answer(base_state, responses):
                    # Combine the original state with the new state from the parser
                    final_state = {**base_state, **new_state}
                    self.thoughts.append(Thought(final_state))
        else:
            # Generate initial thought from kwargs
            base_state = kwargs
            prompt = prompter.generate_prompt(self.num_branches_prompt, **base_state)
            self.logger.debug("Prompt for LM: %s", prompt)
            responses = lm.get_response_texts(
